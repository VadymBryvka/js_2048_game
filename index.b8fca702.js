...
startButton.addEventListener("click", ()=>{
    if (startButton.getAttribute("class") === "button start") {
        ...
        game.start();
    } else // startButton.setAttribute('class', 'button start');
...
document.addEventListener("keydown", (e)=>{
    if (game.getStatus() === "playing") {
        ...
        game.getState();
        if (game.getStatus() === "win") document.querySelector(".message-win").setAttribute("class", "message message-win");
        else if (game.getStatus() === "lose") document.querySelector(".message-lose").setAttribute("class", "message message-lose");
        scoreButton.textContent = game.getScore();
    }
});
 ...
 */ class Game {
   ...
   */ constructor(initialState){
    ...
    rotateGrid(board, clockwise = true) {
        ...
        const newBoard = Array(size).fill(null).map(()=>Array(size).fill(0)); // порожня сітка
        for(let row = 0; row < size; row++){
            for(let col = 0; col < size; col++)if (clockwise) // Обертання на 90 градусів за годинниковою стрілкою
    ...
    getScore() {
        for(let row = 0; row < this.board.length; row++){
            for(let col = 0; col < this.board[row].length; col++)if (this.board[row][col] > this.score) this.score = this.board[row][col];
        }
        return this.score;
    }
    canMove() {
        const copyOfBoard = JSON.parse(JSON.stringify(this.board));
        for(let row = 0; row < copyOfBoard.length; row++)for(let col = 0; col < copyOfBoard[row].length - 1; col++){
            if (copyOfBoard[row][col + 1] === 0 || copyOfBoard[row][col] === 0 || copyOfBoard[row][col] === copyOfBoard[row][col + 1]) return true;
        }
        for(let col = 0; col < copyOfBoard[0].length; col++)for(let row = 0; row < copyOfBoard.length - 1; row++){
            if (copyOfBoard[row + 1][col] === 0 || copyOfBoard[row][col] === 0 || copyOfBoard[row][col] === copyOfBoard[row + 1][col]) return true;
        }
        return false;
    }
    getStatus() {
        ...
        for(let row = 0; row < this.board.length; row++){
            for(let col = 0; col < this.board[row].length; col++)if (this.board[row][col] === 2048) this.status = "win";
        }
